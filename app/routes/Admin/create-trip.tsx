...
import {account} from "~/appwrite/clints";
...
export const loader = async () => {
    try {
        ...
        // إذا data ليس Array → رجّع Array فارغ
        const countriesArray = Array.isArray(data) ? data : [];

        // الآن آمن أن تستخدم map
        return countriesArray.map((country: any) => ({
            name: `${country.flag} ${country.name.common}`,
            coordinates: country.latlng,
            value: country.name.common,
            openStreetMap: country.maps?.openStreetMap,
        ...
        // لو حصل خطأ في fetch أو parse، رجّع Array فارغ
        return [];
    }
};
